...
re<-rep(1, 10)
Re<-data.frame(r, r, r, r, r, r, r, r, r, r)
for (j in c(1:10)) {
  Re[1,j]<--log(subbonds[(10*j-9),4]/((subbonds[(10*j-9),2])/2+100))*2
}
View(Re)

for (j in c(1:10)) {
  for (i in c(2:10)) {
    Re[i,j]<-((subbonds[((10*j-10)+i),4]-sum((subbonds[((10*j-10)+i),2]/2)/((1+R[c(1:(i-1)),j])^(c(1:(i-1))))))*(100+subbonds[((10*j-10)+i),2]/2)^(-1))^(-1/i)-1
  }
}
