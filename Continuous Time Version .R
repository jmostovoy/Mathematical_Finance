...
r_c<-rep(1, 10)
R_c<-data.frame(r, r, r, r, r, r, r, r, r, r)
for (j in c(1:10)) {
  R_c[1,j]<--log(subbonds[(10*j-9),4]/((subbonds[(10*j-9),2])/2+100))*2
}
View(R_c)

for (j in c(1:10)) {
  for (i in c(2:10)) {
    R_c[i,j]<--log((subbonds[((10*j-10)+i),4]-
                      sum((subbonds[((10*j-10)+i),2]/2)*exp(-(i/2)*R_c[c(1:(i-1)),j])))
                   /(100+subbonds[((10*j-10)+i),2]/2))/(i/2)
  }
}

R_c<-as.data.frame(unlist(R_c))
colnames(R_c)<-c("yield")
rownames(R_c) <- seq(length=nrow(R_c))

subbonds_c<-data.frame(subbonds, R_c)
View(subbonds_c)

#### Plots ####

plot_colours <- c("blue", "red", "forestgreen", "yellow", rgb(0.3,0.3,.3), 
                  rgb(0.6,0.3,0), rgb(.9,0,0), rgb(0.3,0.6,0), rgb(0.3,0,.6), 
                  rgb(0,0.3,.6))
plot_colours1 <- plot_colours[c(1,2)]

for (i in c(1:10)) {
  pdf(file=paste("~/Documents/Mathematical_Finance/", paste("Yield_Curve_c_", 
                                                            subbonds_c[10*i-9,6], 
                                                            ".pdf", sep=""), sep=""), 
      width = 10, height = 6)
  plot(subbonds_c[c((10*i-9):(10*i)),3], 100*subbonds_c[c((10*i-9):(10*i)),7], 
       type="l", col=plot_colours1[1], ann=FALSE)
  title(main=paste("Cont. Yield Curve for ", subbonds_c[10*i-9,6], sep=""), 
...
plot(subbonds_c[c((1):(10)),3], 100*subbonds_c[c(1:10),7], type="l", 
     col=plot_colours[1], ann=FALSE)
lines(subbonds_c[c((10*2-9):(10*2)),3], 100*subbonds_c[c((10*2-9):(10*2)),7], type="l", 
      lty=1, col=plot_colours[2])
lines(subbonds_c[c((10*3-9):(10*3)),3], 100*subbonds_c[c((10*3-9):(10*3)),7], type="l", 
      lty=1, col=plot_colours[3])
lines(subbonds_c[c((10*4-9):(10*4)),3], 100*subbonds_c[c((10*4-9):(10*4)),7], type="l", 
      lty=1, col=plot_colours[4])
lines(subbonds_c[c((10*5-9):(10*5)),3], 100*subbonds_c[c((10*5-9):(10*5)),7], type="l", 
      lty=1, col=plot_colours[5])
lines(subbonds_c[c((10*6-9):(10*6)),3], 100*subbonds_c[c((10*6-9):(10*6)),7], type="l", 
      lty=1, col=plot_colours[6])
lines(subbonds_c[c((10*7-9):(10*7)),3], 100*subbonds_c[c((10*7-9):(10*7)),7], type="l", 
      lty=1, col=plot_colours[7])
lines(subbonds_c[c((10*8-9):(10*8)),3], 100*subbonds_c[c((10*8-9):(10*8)),7], type="l", 
      lty=1, col=plot_colours[8])
lines(subbonds_c[c((10*9-9):(10*9)),3], 100*subbonds_c[c((10*i-9):(10*9)),7], type="l", 
      lty=1, col=plot_colours[9])
lines(subbonds_c[c((10*10-9):(10*10)),3], 100*subbonds_c[c((10*i-10):(10*10)),7], type="l", 
      lty=1, col=plot_colours[10])

title(main="All Yield Curves (Cont.)", 
      col.main="forestgreen", font.main=4)
title(xlab="Date", col.lab=rgb(0,0.6,.7))
title(ylab="Yield (in %)" , col.lab=rgb(0,0.6,.7))
legend(as.Date("2017-08-01"), 1.2, paste("Cont. Yield Curve for ", 
                                         subbonds_c[c(1,11,21,31,41,51,61,71,81,91),6], sep=""), 
       lty=c(1,1), 
...
legend(3.8, -.3, c(paste("Forward Curve for ", subbonds_c[c(1,11,21,31,41,51,61,71,81,91),6],
